    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(this,&MainWindow::sendSignal,this,&MainWindow::recSlot);

    //初始化共享内存
    sharememory = new QSharedMemory("lv5y9K3Q7ALwscME",this);
    if(!sharememory->create(230, QSharedMemory::ReadWrite)){
        qDebug() << "创建共享内存失败: " << sharememory->errorString();
        return;
    }
...
/*
* 自创建以后，该进程与共享内存始终处于attach状态，
* 故不用担心，在读进程detach后共享内存销毁
*/
...
{
//    QString str = "testteste";
    QByteArray jsonData = startTracking(0x0001|0x0008);//str.toUtf8();
    ...
    char* sm = static_cast<char*>(sharememory->data());//共享内存映射首地址
    memcpy(sm, jsonData.data(), static_cast<size_t>(qMin(sharememory->size(),jsonData.size())));
    qDebug()<<"set in sharedMemory:"<<jsonData.data();
    ui->listWidget_2->addItem(jsonData.data());

    sharememory->unlock();
}

//button TrackSimulate
void MainWindow::on_pushButton_3_clicked()
{
    QByteArray jsonData = startTrackingSimulate();
...
QByteArray MainWindow::startTracking(int replyOption)
{
...
//----------------------------自此以上写一个json出来-----------------------//
    ...
    return dataJsonDocBA;
}

QByteArray MainWindow::startTrackingSimulate()
{
    //见Polaris_API_Guide.pdf TX指令
 ...
 */
void MainWindow::sleepSeconds(unsigned numSeconds)
{
#ifdef _WIN32
...
bool MainWindow::onErrorPrintDebugMessage(std::string methodName, int errorCode)
{
    if (errorCode < 0)
    {
...
bool MainWindow::configurePassiveTools()
{
...
bool MainWindow::initializeAndEnableTools()
{
...
*/
std::string MainWindow::getToolInfo(std::string toolHandle)
{
...
std::string MainWindow::toolDataToCSV(const ToolData &toolData)
{
...
void MainWindow::writeCSV(std::string fileName, int numberOfLines)
{
...
void MainWindow::printToolData(const ToolData &toolData)
{
...
void MainWindow::printTrackingData()
{
    options = 0x0001;
...
bool MainWindow::loadTool(const char *toolDefinitionFilePath)
{
...
void MainWindow::configureUserParameters()
{
...
void MainWindow::simulateAlerts(uint32_t simulatedAlerts)
{
...
void MainWindow::determineApiSupportForBX2()
{
